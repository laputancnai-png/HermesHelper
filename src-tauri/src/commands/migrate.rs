...
pub(crate) fn export_data_to(
    ...
    let tmp_path = save_path.with_file_name(format!("{}.tmp", file_name));
    ...
    let result = (|| -> Result<(), String> {
        ...
        for item in items {
            match item.as_str() {
                "all" | "data" => {
                    add_hermes_tree_to_zip(&mut zip, hermes_base, options, include_api_keys)?;
                }
                "config" => {
                    // config.toml (legacy)
                    ...
                    // config.yaml (current Hermes format)
                    let yaml_path = hermes_base.join("config.yaml");
                    if yaml_path.exists() {
    ...
    std::fs::rename(&tmp_path, save_path)
        .map_err(|e| format!("Cannot finalize zip file: {e}"))?;
    Ok(())
}

fn should_skip_full_backup_path(relative: &Path) -> bool {
    match relative.components().next() {
        Some(std::path::Component::Normal(name)) => name == "hermes-agent",
        _ => false,
    }
}

fn add_hermes_tree_to_zip(
    ...
    if !hermes_base.exists() || !hermes_base.is_dir() {
        return Ok(());
    }

    let mut stack = vec![hermes_base.to_path_buf()];
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries {
            ...
            let path = entry.path();
            let relative = path
                .strip_prefix(hermes_base)
                .map_err(|e| format!("Path strip error: {e}"))?;
            if should_skip_full_backup_path(relative) {
                continue;
            }
            if path.is_symlink() {
                continue;
            ...
            if zip_name == ".env" {
                let raw = std::fs::read_to_string(&path)
                    .map_err(|e| format!("Cannot read .env: {e}"))?;
                let content = if include_api_keys { raw } else { filter_api_keys(&raw) };
...
mod tests {
    ...
    #[test]
    fn test_export_all_includes_full_hermes_tree_with_filtered_env() {
        let src = tempdir().unwrap();
        std::fs::write(src.path().join("config.yaml"), b"model:\n  provider: nvidia\n").unwrap();
        std::fs::create_dir_all(src.path().join("memory")).unwrap();
        std::fs::write(src.path().join("memory").join("note.md"), b"hello").unwrap();
        std::fs::write(
            src.path().join(".env"),
        ...
        export_data_to(&["all".to_string()], false, &zip_path, src.path()).unwrap();
        ...
        assert!(names.contains(&"memory/note.md".to_string()));
        assert!(names.contains(&".env".to_string()));

        let mut env_file = archive.by_name(".env").unwrap();
        let mut content = String::new();
        std::io::Read::read_to_string(&mut env_file, &mut content).unwrap();
        assert!(!content.contains("NVIDIA_API_KEY"));
        assert!(content.contains("OTHER_VAR=keep"));
    }

    #[test]
    fn test_export_all_skips_hermes_agent_codebase() {
        let src = tempdir().unwrap();
        std::fs::create_dir_all(src.path().join("hermes-agent").join("venv")).unwrap();
        std::fs::write(
            src.path().join("hermes-agent").join("venv").join("big.bin"),
            b"should-not-export",
        )
        .unwrap();
        std::fs::write(src.path().join("config.yaml"), b"ok").unwrap();

        let dst = tempdir().unwrap();
        let zip_path = dst.path().join("backup.zip");
        export_data_to(&["all".to_string()], true, &zip_path, src.path()).unwrap();
        ...
        assert!(names.contains(&"config.yaml".to_string()));
        assert!(!names.iter().any(|n| n.starts_with("hermes-agent/")));
    }
